...
namespace sbt {
    ...
    Client::run()
    {
        ...
        struct timeval interval = {(long)m_interval, 0};
        
        while (true)
        {
            readFds = tmpFds;
            interval.tv_sec = m_interval;
            
            tv.tv_sec = new_tv.tv_sec - old_tv.tv_sec;

            timeval_subtract (&tv, &new_tv, &old_tv);

            timeval_subtract (&tv, &interval, &tv);
                ...
                for(int fd = 0; fd <= maxSockfd; fd++)
                {
                    ...
                    if (FD_ISSET(fd, &readFds))
                    {
                        ...
                        if (fd == sockfd_listen) // this is the listen socket
                        {
                            ...
                            fcntl(clientSockfd, F_SETFL, flags);
                        ...
                        else if (fd == m_trackerSock)
                        {
                            ...
                            if (good_response)
                            {
                                ...
                                for (const auto& peer : m_peerlist)
                                {
                                    ...
                                    int flags1 = fcntl(peer_socket, F_GETFL, 0);

                                    flags1 &= ~O_NONBLOCK;

                                    fcntl(peer_socket, F_SETFL, flags1);
                                    ...
                                    for (unsigned i = 0 ; i < m_pieceNeed[it->first].size(); i ++)
                                    {
                                        ...
                                        if (m_requested.find(m_pieceNeed[it->first][i]) == m_requested.end())
                                        {
                                            ...
                                            if ((int)m_pieceNeed[it->first][i] == m_numPieces - 1)
                                            {
                                                ...
                                                int flags = fcntl(it->first, F_GETFL, 0);
    ...
    Client::broadcastHaveMsg(int pieceIndex) 
    {
        ...
        {
            int flags = fcntl(it->second, F_GETFL, 0);
    ...
    int Client::timeval_subtract (struct timeval *result, struct timeval *x, struct timeval *y)
    {
